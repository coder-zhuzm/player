...
- **歌词时间轴微调工具**：支持 `[-0.1s]` 延后 / `[+0.1s]` 提前 / `[重置 0s]` 时间轴校准。
...
- **妖狐 API 支持**：内置 100ms 请求限流器，支持 URL 自动追链与 `localStorage` Key 本地持久化。
...
3. Key 将存放在当前浏览器的 `localStorage` 中，后续操作将自动读取调用；请勿在公共设备上保存私人 Key。
